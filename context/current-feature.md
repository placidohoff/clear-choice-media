...
- Goal: Per @context/mobile-optimization.md (this one lives directly under `context/`, not `context/features/` like the others), fix two mobile issues: (1) around 400px screen width, the contact form and other content gets cut off; (2) the mobile nav menu can't be scrolled when its content is taller than the screen — scrolling on the open menu scrolls the page behind it instead.
- Diagnosed but couldn't pinpoint the exact overflowing element via static code analysis alone (no real device/browser tool available) — instead of guessing which element to shrink, changed the root `overflowX: 'hidden'` (in `app/page.tsx`) to `'auto'`, which is the fix the user explicitly suggested. This makes any genuinely-overflowing content reachable via horizontal scroll without risking the decorative blur/glow effects elsewhere (CSS blur filters don't count toward scrollable overflow, so nothing new should show a scrollbar unless something is actually too wide).
- Mobile menu fix (`app/components/NavHeader.tsx`): the overlay panel had no max-height/scroll container of its own, and nothing prevented touch-scroll from falling through to the page behind it. Added a capped, internally-scrollable container plus a body-scroll lock (toggling `document.body.style.overflow`) while the menu is open.
- Bundled ad-hoc requests onto this branch (per user, mid-flow):
	- Removed the business phone number/call option from public display entirely (`app/page.tsx` — the contact section's info list and the footer's `tel:` "Call" link). The `ContactForm`'s own Phone input (collecting the *visitor's* number) is unrelated and untouched. Underlying `contact.phone` data field kept in the schema/data in case it's wanted back later — only the display was removed.
	- Removed the business email (`Clifton@clearchoicemedia.co`) from public display entirely, same reasoning — the client wants visitors to use the contact form exclusively, not call/email directly. Backend use (Resend still sends notification emails *to* that address) is untouched; only public display removed. The admin dashboard (behind login) still shows it internally, unaffected.
	- Redesigned the footer's now-sparse "Contact" column (after removing phone/email) into a region line ("Rhode Island and Southern New England" — a literal string in this one footer spot, not the shared `contact.city`/`contact.serviceArea` data fields, to avoid duplicating with the main contact section's display of those) plus a "Get In Touch →" link to `/#contact`.
- Verified live on a local dev server at each step (overflow-x change, mobile menu scroll classes, phone/email removal, footer redesign) since no browser/device tool was available to visually confirm — user confirmed the actual mobile behavior and appearance directly.
